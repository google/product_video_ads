...
class VideoGenerator(object):
  ...
  def __init__(self):
    ...
    # Imagemagick run from Docker (not used anymore)
    '''self.convert_executable = ['docker', 'run', '--rm',
          '-v', '%s:%s' % (os.path.expanduser('~'), os.path.expanduser('~')),
                               '-v', '/tmp:/tmp',
                               'imagemagick',
                               '/home/imagemagick/install/bin/convert']
    '''
  ...
  def process_video(self, image_or_videos_overlays, text_overlays, input_video,
    ...
    # Validate first:
    self._validate_overlay_times(input_video,
                                 image_or_videos_overlays + text_overlays)
  ...
  def _audio_inputs(self, audio_files, files_index):
    # http://superuser.com/questions/769168/ffmpeg-mixing-and-setting-each-volume-of-a-multi-audio-track-file
    audio_args = []
    audio_overlay = []

    # makes base video the initial overlay
    initial_ovl = '[0:a]aformat=sample_fmts=fltp:sample_rates=44100:'
    initial_ovl += 'channel_layouts=stereo,volume=1.0[audout0]'
    audio_overlay.append(initial_ovl)
    i = 0
    out_index = '0'

    # for each audio file...
    for i, a in enumerate(audio_files):
      filename = util.fix_asset_relative_path(a['audio'])

      # set up insert audio file command
      duration = str(float(a['end_time']) - float(a['start_time']))
      audio_args += ['-itsoffset', str(a['start_time']), '-t', duration, '-i']
      audio_args += ['%s/' % (filename)]

      # set up filter
      out_index = (i + 1)
      audio_file_index = (int(files_index) + int(out_index))
      a_ovl = ''

      # format current audio file
      a_ovl += '[%s:a]aformat=sample_fmts=fltp:sample_rates=44100:'
      a_ovl += 'channel_layouts=stereo,volume=%s [audformated%s];'
      a_ovl %= (audio_file_index, float(a['volume']) / 100, out_index)

      # makes audio start when supposed
      delay = float(a['start_time']) * 1000 + 1
      a_ovl += '[audformated%s]adelay=%s|%s[aud%s];'
      a_ovl %= (out_index, delay, delay, out_index)

      # overlays this audio on previous overlays
      a_ovl += ('[audout%s][aud%s] amix=inputs=2:duration=first[audout%s]')
      a_ovl %= (i, out_index, out_index)

      audio_overlay.append(a_ovl)

    # makes last overlay as final overlay to be used in final video
    self.out_audio = '[audout%s]' % (out_index)

    # returns created overlays and insert commands
    return audio_args, audio_overlay
