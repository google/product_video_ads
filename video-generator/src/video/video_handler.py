...
class VideoHandler():
  ...
  HANDLED_STATUS = ['Products Ready', 'Paused', 'Preview']
  ...
  def handle_configuration(self):
    ...
    # Configuration for all base videos (which products to show up each place)
    base_configs = self.configuration.get_base_config(base_videos)
    ...
    for row, campaign in enumerate(campaign_config):
      ...
      (product_ids, base_name, status) = campaign
      ...
      config = {
          ...
          'base_config': base_configs.get(base_name),
          'product_ids': [p.strip() for p in product_ids.split(',') if p],
