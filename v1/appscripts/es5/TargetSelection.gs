...
function getValidationData() {
  ...
  try {
    const customer_targets = SpreadsheetApp.getActiveSpreadsheet()
    .getRange("Target!A2:A")
    .getValues()
    .map(function(e) {
      return e[0]
    })
    
    const custom_targets = SpreadsheetApp.getActiveSpreadsheet()
    .getRange("Target!B2:B")
    .getValues()
    .map(function(e) {
      return e[0]
    })
